...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        if (!isset($_FILES['avatarFile']) || $_FILES['avatarFile']['error'] !== 0) {
            $query = $writeDb->prepare('UPDATE customer SET `name`=:username,`phone`=:phone where id=:user_id');
            ...
            if (move_uploaded_file($_FILES['avatarFile']['tmp_name'],  $file_path)) {
                ...
                $query = $writeDb->prepare('UPDATE customer SET `name`=:username,`phone`=:phone,`avatar`=:avatar where id=:user_id');
                $query->bindParam(':user_id', $userId, PDO::PARAM_STR);
                $query->bindParam(':username', $username, PDO::PARAM_STR);
                $query->bindParam(':phone', $phone, PDO::PARAM_STR);
